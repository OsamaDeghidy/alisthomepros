...
const GoldText = ({ children, className = "" }: { children: React.ReactNode, className?: string }) => (
  <span className={`text-transparent bg-clip-text bg-gradient-to-r from-[#B8960C] via-[#E2C044] to-[#B8960C] font-black ${className}`}>
    {children}
  </span>
);
...
export default function GuestHomePage() {
  ...
  const getRegisterUrl = (role: string) => {
    const base = APP_GATEWAY_URL.endsWith('/') ? APP_GATEWAY_URL.slice(0, -1) : APP_GATEWAY_URL;
      ...
      <section className="py-32 px-4 bg-white relative overflow-hidden">
         <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
               {audiences.filter(a => a.slug === 'pro' || a.slug === 'crew').map((audience) => (
                  <a 
                    key={audience.id}
                    href={getRegisterUrl(audience.slug)}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="relative aspect-[4/5] rounded-[3.5rem] overflow-hidden group/card block border border-gray-100 shadow-2xl"
                  >
                     <Image 
                       src={audience.image} 
                       alt={audience.title}
                     ...
                     <div className="absolute inset-0 p-10 flex flex-col justify-end">
                        ...
                        <h3 className="text-3xl font-black text-white mb-4 uppercase italic">{audience.title}</h3>
                        <p className="text-white/70 font-medium leading-relaxed mb-8 line-clamp-2 md:line-clamp-3">{audience.desc}</p>
                        <div className="flex items-center text-white font-black text-sm uppercase tracking-widest bg-white/10 w-fit px-6 py-3 rounded-xl border border-white/10 backdrop-blur-sm group-hover/card:bg-primary-600 group-hover/card:border-primary-500 transition-all italic">
                           {audience.cta}
            ...
            <div className="mt-16 max-w-2xl mx-auto">
               <button 
                  ...
                  className="w-full flex items-center justify-between p-6 bg-gray-50 rounded-2xl font-black uppercase italic tracking-widest text-gray-500 hover:bg-gray-100 transition-all border border-gray-100"
               >
                  <span>More Ways to Join</span>
                  <Zap className={`w-5 h-5 transition-transform duration-300 ${isAccordionOpen ? 'rotate-180 text-primary-600' : ''}`} />
               </button>
                 ...
                 {isAccordionOpen && (
                   ...
                   >
                     <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                        {audiences.filter(a => a.slug === 'specialist' || a.slug === 'partner').map((audience) => (
                           <a 
                              key={audience.id}
                              href={getRegisterUrl(audience.slug)}
                              target="_blank"
                              rel="noopener noreferrer"
                              className="p-6 bg-white border border-gray-100 rounded-3xl hover:border-primary-500 transition-all group"
                           >
                              <div className="flex items-center gap-4 mb-4">
                                 <div className="w-10 h-10 bg-primary-50 rounded-xl flex items-center justify-center">
                                    <audience.icon className="w-5 h-5 text-primary-600" />
                                 </div>
                                 <h4 className="font-black uppercase italic text-gray-900">{audience.title}</h4>
                               </div>
                               <p className="text-xs text-gray-500 font-medium mb-4">{audience.desc}</p>
                               <div className="text-[10px] font-black uppercase text-primary-600 flex items-center">
